            ...
            <tbody class="bg-slate-800 divide-y divide-slate-700">
                @forelse($inspections as $inspection)
                    <tr class="hover:bg-slate-700 transition">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-slate-100">{{ $inspection->loan->employee->name ?? 'N/A' }}</td>
                ...
                @empty
                    <tr>
                        <td colspan="5" class="px-6 py-10 text-center text-slate-400">No assets pending inspection.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
...
<div x-show="showModal" x-cloak class="fixed inset-0 z-50 overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true">
    <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
        ...
        <div x-show="showModal" 
            ...
            <form method="POST" :action="'/inspections/' + selectedInspectionId + '/update'">
                @csrf
                        ...
                        <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left w-full">
                            ...
                            <div class="mt-2">
                                <p class="text-sm text-slate-400">
                                    Asset: <span x-text="selectedAsset" class="text-slate-100 font-bold"></span>
                                </p>
                            </div>
                            <div class="mt-4 space-y-4">
                                <!-- Technical Condition - Matches Condition Enum -->
                                <div>
                                    <label for="condition" class="block text-sm font-medium text-slate-300">Technical Condition</label>
                                    <select id="condition" name="condition" required class="mt-1 block w-full bg-slate-700 border border-slate-600 rounded-md shadow-sm py-2 px-3 text-slate-100 focus:outline-none focus:ring-blue-500 focus:border-blue-500">
                                ...
                                <!-- New Status - Matches AssetStatus Enum -->
                                <div>
                                    <label for="status" class="block text-sm font-medium text-slate-300">New Status</label>
                                    <select id="status" name="status" required class="mt-1 block w-full bg-slate-700 border border-slate-600 rounded-md shadow-sm py-2 px-3 text-slate-100 focus:outline-none focus:ring-blue-500 focus:border-blue-500">
                                ...
                                <div>
                                    ...
                                    <textarea id="notes" name="notes" rows="2" class="mt-1 block w-full bg-slate-700 border border-slate-600 rounded-md shadow-sm py-2 px-3 text-slate-100 focus:outline-none focus:ring-blue-500 focus:border-blue-500" placeholder="Any technical remarks..."></textarea>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div class="bg-slate-700 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
                    ...
                    <button type="button" @click="closeModal" class="mt-3 w-full inline-flex justify-center rounded-md border border-slate-600 shadow-sm px-4 py-2 bg-slate-700 text-base font-medium text-slate-300 hover:bg-slate-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-slate-500 sm:mt-0 sm:ml-3 sm:w-auto sm:text-sm transition-colors">
                        Cancel
                    </button>
                </div>
            </form>
